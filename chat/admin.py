...
class UserProfileAdmin(admin.ModelAdmin):
    ...
    def display_image(self, image_field, small=False):
        if image_field:
            size = '50px' if small else '150px'  # Adjust sizes as needed
            return format_html('<img src="{}" style="width: {}; height: auto;"/>', image_field.url, size)
        return "No Image"
    ...
    class UserProfileAdmin(admin.ModelAdmin):
        ...
        def get_form(self, request, obj=None, **kwargs):
            # Dynamically adjust the excluded fields based on the instance
            if obj and not obj.is_bot:
                # If it's not a bot, prepare to exclude bot-specific fields
                self.form.Meta.exclude = ('grounding_source', 'stock_answer', 'personalities')
            else:
                # Reset excluded fields when it is a bot or creating a new instance
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'email', 'username', 'avatar', 'is_bot', 'grounding_source', 'stock_answer', 'display_avatar_large')
        }),
